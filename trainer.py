...
def _train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        print(f"cnn_accy is {cnn_accy}")
        print(f"nme_accy is {nme_accy}")
        model.after_task()
    ...
    if len(cnn_matrix)>0:
        if args['init_cls'] != args['increment']:
            np_acctable = np.zeros([task + 1,int(100/args['increment']) ])
        else:
            np_acctable = np.zeros([task + 1,task + 1 ])
            
        for idxx, line in enumerate(cnn_matrix):
            idxy = len(line)
            print(f"iddx is {idxx} , idxy is {idxy} , line is {line}")
